...
namespace KinectSabre.Render
{
    public class BloomComponent : DrawableGameComponent
    {
        ...
        public BloomComponent(Game game, float red, float blue, float green)
            : base(game)
        {
            if (game == null)
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            bloomExtractEffect.Parameters["BloomThreshold"].SetValue(0.25f);
            ...
            parameters["BloomIntensity"].SetValue(4);
            parameters["BaseIntensity"].SetValue(1);
            parameters["BloomSaturation"].SetValue(2);
            parameters["BaseSaturation"].SetValue(0);
            parameters["r"].SetValue(r);
            parameters["b"].SetValue(b);
            parameters["g"].SetValue(g);


            GraphicsDevice.Textures[1] = sceneRenderTarget;
            GraphicsDevice.Textures[2] = ColorTexture;

            Viewport viewport = GraphicsDevice.Viewport;

            spriteBatch.Begin(0, BlendState.Opaque, null, null, null, bloomCombineEffect);
            spriteBatch.Draw(renderTarget1, new Rectangle(0, 0, viewport.Width, viewport.Height), Color.White);
            spriteBatch.End();
        }
        ...
        /// </summary>
        static float ComputeGaussian(float n, float theta = 4)
        {
